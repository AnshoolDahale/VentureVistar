...
export const fetchCart = () => async dispatch => {
  try {
    const {data: stringifyData, statusCode} = await Api.getRequest(`/api/cart/`)
    
    if (statusCode !== 200) {
      console.error('Failed to fetch cart:', stringifyData)
      return
    }
    ...
    if (typeof stringifyData === 'string') {
      ...
      parsedData = stringifyData
    }
    
    const {carts} = parsedData
